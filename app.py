...
def login():
    ...
    # Verifica se o usuário existe, se sim, depois verifica se a senha que o usuário enviou é a mesma
    # cadastrada no banco de dados
    if user and data.get("password") == user.password:
            login_user(user)
            return jsonify({"Message": "Logado com Sucesso!"})
    
    return jsonify({"Message": "Não autorizado. Senha incorreta!"}), 401
...
def add_product():
    ...
    # Verifica se as chaves existem no json informado, caso existam o produto é cadastrado, caso não, é retornado mensagem de erro
...
def update_product(product_id):
    product = Product.query.get(product_id)
    # Condicão que verifica se o produto não existe, caso seja verdadeira retorna a mensagem "Produto não encontrado!"
    if not product:
        return jsonify({"Message": "Produto não encontrado!"}), 404
